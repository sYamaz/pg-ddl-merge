...
| コマンド | 状況 | 備考 |
|---------|------|------|
| `CREATE TABLE` | ✅ | 基本カラム定義・NOT NULL・DEFAULT・制約（名前付き/無名）・TEMP/UNLOGGED/PARTITION BY/LIKE に対応。INHERITS/WITH/TABLESPACE/ON COMMIT/USING は構文解析のみ（スキーマ追跡への影響なし） |
| `ALTER TABLE` | 🔶 | ADD/DROP COLUMN・ALTER COLUMN TYPE/DEFAULT/NOT NULL・RENAME COLUMN/TO・ADD/DROP CONSTRAINT（名前付き/無名）・ADD GENERATED AS IDENTITY・DROP IDENTITY・SET STORAGE・SET COMPRESSION に対応。ONLY 修飾子はパース時に無視。未認識アクションは警告を出してスキップ（エラーにならない） |
...
| コマンド | 状況 | 備考 |
|---------|------|------|
| `CREATE INDEX` | ✅ | UNIQUE・CONCURRENTLY・IF NOT EXISTS をフィールドとして追跡・出力。同名インデックスの再定義は後勝ち（警告付き）。USING method・WHERE（部分インデックス）・INCLUDE・WITH（storage）・TABLESPACE は Body に verbatim 保持 |
...
| コマンド | 状況 | 備考 |
|---------|------|------|
| `CREATE SEQUENCE` | ✅ | IF NOT EXISTS 対応・名前抽出・本体オプションを個別フィールドとして保持（多行対応） |
| `ALTER SEQUENCE` | ✅ | IF EXISTS 修飾子対応・RENAME TO・INCREMENT BY/MINVALUE/MAXVALUE/START WITH/CACHE/CYCLE/OWNED BY/AS/SET LOGGED に対応。RESTART はパース済み（runtime 状態のため body 更新なし） |
...
| `CREATE OR REPLACE FUNCTION` | ✅ | OR REPLACE で上書き（PostAlters もリセット） |
| `ALTER FUNCTION` | ✅ | RENAME TO・OWNER TO・SECURITY DEFINER/INVOKER などの非 RENAME アクションに対応。対象関数が schema 内に存在する場合は `PostAlters` として紐付けられ CREATE の直後に出力。存在しない場合は末尾にパススルー |
| `DROP FUNCTION` | ✅ | IF EXISTS・名前抽出（`(` 前まで）に対応 |
| `CREATE PROCEDURE` | ✅ | verbatim 保持 |
| `ALTER PROCEDURE` | ✅ | `ALTER FUNCTION` と同様（RENAME TO・OWNER TO・SECURITY など全非 RENAME アクション対応） |
...
| `ALTER EXTENSION` | ✅ | RENAME TO・UPDATE [TO version] に対応（PostAlter として CREATE 直後に出力）。ADD MEMBER / DROP MEMBER は DBA 操作としてスコープ外・UnknownStmt でパススルー |
...
| `ALTER DOMAIN` | ✅ | RENAME TO・ADD/DROP CONSTRAINT・SET/DROP DEFAULT・SET/DROP NOT NULL などの内容変更アクションに対応（PostAlter として CREATE 直後に出力）。OWNER TO / SET SCHEMA は警告を出してスキップ |
...
| `ALTER POLICY` | ✅ | RENAME TO・TO roles / USING / WITH CHECK などの内容変更アクションに対応（PostAlter として CREATE 直後に出力）。ポリシーが schema 内に存在しない場合は末尾にパススルー |
...
| `ADD PRIMARY KEY / UNIQUE / FOREIGN KEY / CHECK`（無名） | ✅ | 名前なし制約の追加 |
| `DROP CONSTRAINT [IF EXISTS] name` | ✅ | |
| 複数アクション（カンマ区切り） | ✅ | |
| `ONLY` 修飾子 | ✅ | パース時に無視 |
| `ALTER COLUMN col SET STATISTICS n` | ✅ | 警告を出してスキップ（スキーマモデルに影響なし） |
| `ALTER COLUMN col SET STORAGE type` | ✅ | カラムの InlineConstraints を更新（STORAGE キーワードを置換または追加） |
...
| `ENABLE ALWAYS / REPLICA TRIGGER` | ✅ | 同上 |
| `ENABLE/DISABLE RULE` | ✅ | 同上 |
| `ENABLE ALWAYS / REPLICA RULE` | ✅ | 同上 |
| `ENABLE/DISABLE ROW LEVEL SECURITY` | ✅ | 同上 |
| `FORCE/NO FORCE ROW LEVEL SECURITY` | ✅ | 同上 |
| `SET LOGGED / SET UNLOGGED` | ✅ | 警告を出してスキップ（テーブルの永続性変更はスキーマモデルに反映されない） |
| `ATTACH PARTITION` | ✅ | 警告を出してスキップ |
| `DETACH PARTITION` | ✅ | 同上 |
...
| `TRUNCATE TABLE name CONTINUE IDENTITY` | ✅ | パース時にストリップ（`RestartIdentity=false`）・テストあり |
| `TRUNCATE TABLE name CASCADE` | ✅ | |
| `TRUNCATE TABLE name RESTRICT` | ✅ | パース時にストリップ |
