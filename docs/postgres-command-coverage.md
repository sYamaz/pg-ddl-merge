...
| `ALTER TABLE` | 🔶 | ADD/DROP COLUMN・ALTER COLUMN TYPE/DEFAULT/NOT NULL・RENAME COLUMN/TO・ADD/DROP CONSTRAINT・ADD GENERATED AS IDENTITY・DROP IDENTITY・SET STORAGE・SET COMPRESSION に対応。未認識アクションは警告を出してスキップ（エラーにならない） |
