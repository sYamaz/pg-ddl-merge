...
var (
	reCreateTable    = regexp.MustCompile(`(?i)^CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(\S+)\s*\(`)
...
var (
	...
	reInlineKeywords = regexp.MustCompile(`(?i)^(PRIMARY\s+KEY|UNIQUE|FOREIGN\s+KEY|CHECK)\s*`)
)
...
func parseColumnDef(s string) (ColumnDef, error) {
	...
	// Parse DEFAULT
	if idx := indexCI(rest, " DEFAULT "); idx >= 0 {
		before := rest[:idx]
