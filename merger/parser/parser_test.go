...
// ---- ALTER TABLE: new column-level actions ----------------------------------

func TestParseAlterTable_AddGenerated(t *testing.T) {
	cases := []struct {
		sql     string
		col     string
		clause  string
		...
		{
			"ALTER TABLE t ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (START WITH 10 INCREMENT BY 1)",
			"id", "GENERATED BY DEFAULT AS IDENTITY (START WITH 10 INCREMENT BY 1)",
		},
	}
	for _, tc := range cases {
		...
		at, ok := stmt.(AlterTableStmt)
		if !ok {
			t.Fatalf("want AlterTableStmt for %q, got %T", tc.sql, stmt)
		}
		if len(at.Actions) != 1 {
			t.Fatalf("want 1 action, got %d", len(at.Actions))
		}
		a := at.Actions[0]
		if a.Kind != ActionAddGenerated {
			t.Errorf("want ActionAddGenerated, got %v", a.Kind)
		}
		if a.Column != tc.col {
			t.Errorf("Column: want %q, got %q", tc.col, a.Column)
		}
		if a.GeneratedClause != tc.clause {
			t.Errorf("GeneratedClause: want %q, got %q", tc.clause, a.GeneratedClause)
		}
	}
}

func TestParseAlterTable_DropIdentity(t *testing.T) {
	cases := []struct {
		sql      string
		col      string
		ifExists bool
	}{
		{"ALTER TABLE t ALTER COLUMN id DROP IDENTITY", "id", false},
		{"ALTER TABLE t ALTER COLUMN id DROP IDENTITY IF EXISTS", "id", true},
	}
	for _, tc := range cases {
		stmt := mustParse(t, tc.sql)
		at, ok := stmt.(AlterTableStmt)
		...
		a := at.Actions[0]
		if a.Kind != ActionDropIdentity {
			t.Errorf("want ActionDropIdentity, got %v", a.Kind)
		}
		if a.Column != tc.col {
			t.Errorf("Column: want %q, got %q", tc.col, a.Column)
		}
		if a.IfExists != tc.ifExists {
			t.Errorf("IfExists: want %v, got %v", tc.ifExists, a.IfExists)
		}
	}
}

func TestParseAlterTable_SetStorage(t *testing.T) {
	stmt := mustParse(t, "ALTER TABLE t ALTER COLUMN c SET STORAGE EXTERNAL")
	at, ok := stmt.(AlterTableStmt)
	if !ok {
		t.Fatalf("want AlterTableStmt, got %T", stmt)
	}
	a := at.Actions[0]
	if a.Kind != ActionSetStorage {
		t.Errorf("want ActionSetStorage, got %v", a.Kind)
	}
	if a.Column != "c" {
		t.Errorf("Column: want %q, got %q", "c", a.Column)
	}
	if a.StorageType != "EXTERNAL" {
		t.Errorf("StorageType: want %q, got %q", "EXTERNAL", a.StorageType)
	}
}

func TestParseAlterTable_SetCompression(t *testing.T) {
	stmt := mustParse(t, "ALTER TABLE t ALTER COLUMN c SET COMPRESSION pglz")
	at, ok := stmt.(AlterTableStmt)
	if !ok {
		t.Fatalf("want AlterTableStmt, got %T", stmt)
	}
	a := at.Actions[0]
	if a.Kind != ActionSetCompression {
		t.Errorf("want ActionSetCompression, got %v", a.Kind)
	}
	if a.Column != "c" {
		t.Errorf("Column: want %q, got %q", "c", a.Column)
	}
	if a.CompressionMethod != "pglz" {
		t.Errorf("CompressionMethod: want %q, got %q", "pglz", a.CompressionMethod)
	}
}
